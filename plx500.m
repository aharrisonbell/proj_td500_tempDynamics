...
for f=1:length(files), % perform following operations on each nex file listed
    ...
    if length(find(ismember(tempbehav(:,2),parnumlist)))<1,
        ...
        for un=1:foundunits, % performed for each unit
            ...
            [graphstructsingle.faces_avg,graphstructsingle.faces_sem]=plx_avgspden_old(spikestructsingle.faces_spk,spikestructsingle.faces_ts,5000,1,hmiconfig.gausskernel);
            [graphstructsingle.fruit_avg,graphstructsingle.fruit_sem]=plx_avgspden_old(spikestructsingle.fruit_spk,spikestructsingle.fruit_ts,5000,1,hmiconfig.gausskernel);
            [graphstructsingle.bodyp_avg,graphstructsingle.bodyp_sem]=plx_avgspden_old(spikestructsingle.bodyp_spk,spikestructsingle.bodyp_ts,5000,1,hmiconfig.gausskernel);
            [graphstructsingle.places_avg,graphstructsingle.places_sem]=plx_avgspden_old(spikestructsingle.places_spk,spikestructsingle.places_ts,5000,1,hmiconfig.gausskernel);
            [graphstructsingle.objct_avg,graphstructsingle.objct_sem]=plx_avgspden_old(spikestructsingle.objct_spk,spikestructsingle.objct_ts,5000,1,hmiconfig.gausskernel);
            ...
            for tr=1:size(tempspike(un).spikes,1),
                ...
                [graphstructsingle.spden_trial(tr,:),junk]=plx_avgspden_old(tempspikes,temp_ts,5000,1,hmiconfig.gausskernel);
            ...
            for cnd=1:100, % first loop creates average spike density function for each condition
                ...
                if isempty(pointer)==1, % if no trials are found, paste zero values
                    ...
                    [graphstructsingle.allconds_avg(cnd,:),graphstructsingle.allconds_sem(cnd,:)]=plx_avgspden_old(tempspikes,temp_ts,5000,1,hmiconfig.gausskernel);
