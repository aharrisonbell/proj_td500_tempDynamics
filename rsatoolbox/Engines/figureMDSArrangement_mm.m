function figureMDSArrangement_mm(RDM, userOptions, localOptions)
...
% mm:
%   (1) Only set localOptions.figI_textLabels if none of the possible figI 
%       fields (figI_textLabels, figI_catCols, and figI_icons) is defined. 
%       In the original function, figI_textLabels is set if it's undefined
%       (independent of whether other figI fields are defined).
%   (2) If (1) happens, also set localOptions.textLabels if they are not 
%       defined. This field is not set in the original function, which
%       leads to an error if the variable contrast is undefined.
%   (3) Category-colored dots now works too (figures currently saved 
...
if ~isfield(localOptions,'figI_textLabels')&&~isfield(localOptions,'figI_catCols')&&~isfield(localOptions,'figI_icons')
    localOptions=setIfUnset(localOptions,'figI_textLabels',1);
    RDM_sq=unwrapRDMs(squareRDMs(RDM));
    nConditions=size(RDM_sq,1);
    defaultLabels=cell(nConditions,1);
    for i=1:nConditions
        defaultLabels(i)={num2str(i)};
    end
    clear RDM_sq
    localOptions=setIfUnset(localOptions,'textLabels',defaultLabels);    
end
...
if isfield(localOptions,'figI_catCols')&&~isempty(localOptions.figI_catCols) 
    nCategories=size(localOptions.contrasts,2); 
    colormap(jet); jetCols=colormap;
    localOptions=setIfUnset(localOptions,'categoryColors',jetCols(round(linspace(1,64,nCategories)),:)); 
    ...
    if exist('pats_mds_2D','var')
        plotDots(pats_mds_2D,localOptions.contrasts(:,localOptions.categoryIs),localOptions.categoryColors(localOptions.categoryIs,:),localOptions.categoryLabels(localOptions.categoryIs));         
...
if isfield(localOptions,'figI_icons')&&~isempty(localOptions.figI_icons) 

    figIs=[figIs,pageFigure(localOptions.figI_icons(1))]; [hf,ha]=selectPlot(localOptions.figI_icons); 
    ...
    drawImageArrangement(localOptions.icons,pats_mds_2D); 
    axis tight equal off;
